...
There are three distinct "empty" scenarios — distinguish between them clearly:

1. **No task stages at all** (`task_stages` array is empty, `total_tasks` is 0): The workspace has no run tasks configured. Tell the user: "This run has no run tasks configured."

2. **Task stages exist but contain zero task results** (`task_stages` is non-empty but each stage's `task_results` array is empty, `total_tasks` is 0): The run task infrastructure exists but produced no individual results. Tell the user: "This run has task stages but no task results were produced. The stages are: [list stage names and statuses]."
