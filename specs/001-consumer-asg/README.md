...
- exposes an internal HTTPS Application Load Balancer (ALB) reachable only from approved VPC CIDRs
...
|------------------|--------|---------|---------|
| `module.alb` | `app.terraform.io/hashi-demos-apj/alb/aws` | `~> 10.1` | Internal HTTPS ALB, listener, and target group |
...
5. You have at least one issued ACM certificate in `ap-southeast-2` for the DNS name clients will use to reach the internal ALB.
...
- `certificate_domain` to narrow automatic ACM discovery to a specific domain or wildcard name
...
`certificate_arn` is optional. If it is omitted or set to `null`, Terraform automatically discovers the most recent `ISSUED` ACM certificate in `ap-southeast-2`. Set `certificate_domain` as an optional filter when you want discovery constrained to a specific certificate name.
...
1. Prepare input values.
    ...
    - Otherwise leave `certificate_arn = null` and optionally set `certificate_domain` so Terraform can auto-discover a matching issued ACM certificate.
...
- HTTPS is always enabled on the ALB listener by using `certificate_arn` when provided or an automatically discovered issued ACM certificate in `ap-southeast-2` when it is not.
